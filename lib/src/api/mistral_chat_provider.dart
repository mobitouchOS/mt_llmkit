// lib/src/api/mistral_chat_provider.dart
...
///
/// Mistral uses an OpenAI-compatible Chat Completions endpoint, so the
/// request/response format is identical to [OpenAIChatProvider].
///
/// Supported config keys:
/// - `apiKey` *(required)*: Mistral API key
/// - `model` *(optional)*: model name (default: `"mistral-small-latest"`)
///
/// Example:
/// ```dart
...
///   'Summarise the history of the Eiffel Tower in two sentences.',
/// );
/// print(response.message.content);
///
/// await provider.dispose();
/// ```
class MistralChatProvider extends BaseAIChatProvider {
  String _apiKey = '';
  ...
  Future<void> initialize(Map<String, dynamic> config) async {
    ...
    markAsInitialized();
    dev.log('Initialized with model=$_model', name: 'MistralChatProvider');
  }
  ...
  Future<void> dispose() async {
    ...
    _httpClient = null;
    await super.dispose();
    dev.log('Disposed.', name: 'MistralChatProvider');
  }
